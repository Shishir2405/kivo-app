...
/**
 * A flat Steep topic card: a small thin glyph + serif title, a one-line
 * description, difficulty + mastery tags, a solved/total counter, and a thin
 * Ink/Rust progress rail. Dove hairline + the one subtle shadow.
 */
export function TopicCard({ topic, onPress, index = 0 }: TopicCardProps) {
  ...
  const mastery = masteryMeta(topic.mastery ?? progress);
        ...
        <SoftCard radius={16} padding={14}>
          <View style={{ gap: 12 }}>
            ...
            <View className="flex-row items-center" style={{ gap: 10 }}>
              <Icon name={topic.emoji} size={17} color="ink" />
